...
;// CONCATENATED MODULE: ./model/category.js
...
const Category = (external_mongoose_default()).models.Category || external_mongoose_default().model("Category", categorySchema);
/* harmony default export */ const model_category = (Category);

;// CONCATENATED MODULE: ./pages/api/admin/category/index.js
...
async function handler(req, res) {
    ...
    if (method == "GET") {
        ...
        try {
            // Find a user by ID in the database
            const categoryData = await model_category.findOne({
            ...
            const category = new model_category({
                category: updateCategory,
                image: image
            });
        ...
        try {
            const { id  } = req.query;
            const deletedCategory = await model_category.findByIdAndDelete(id);
            if (!deletedCategory) {
                res.status(404).json({
